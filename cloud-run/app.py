...
def get_top5_moves(recommended_list):
    # 推奨行動を「適用距離」で昇順にソート
    sorted_list = sorted(recommended_list, key=lambda x: x.get("適用距離", float('inf')))
...
def calc_distance():
    ...
    def moves_to_html(moves):
      ...
      move_descriptions = {
        # Mario 特殊技
        "side_b_weapon": "側面特殊攻撃（火の玉）",
        "down_b_weapon": "下方特殊攻撃（ボムなど）",
        "jab_startup": "ジャブ攻撃（素早い近接パンチ）",
        "up_b_startup": "上方特殊攻具（回転ジャンプ攻撃）",
        "down_b_startup": "下方特殊攻撃（ダウン攻撃）",
        
        # Link 特殊技
        "up_b_weapon": "上方特殊攻撃（回転攻撃）",
        "side_b_tobi": "側面特殊攻撃（飛び道具）",
        "neutral_b_tobi": "中性特殊攻撃（飛び道具）",
        "uair_weapon": "空中上攻撃（武器使用）",
        "fair_weapon": "空中前方攻撃（武器使用）",
        # Link 特殊技
        "uair_x": "空中上攻撃（横方向）",
        "nair_y": "空中中性攻撃（垂直方向）",
        "y": "ダッシュ攻撃（垂直方向）",
      ...
      return "<ul>" + "".join(
        f"<li>{move['カテゴリ']} - {move_descriptions.get(move['行動'], move['行動'])} (有効距離: {move['適用距離']}マス以下)</li>" 
        for move in moves
    ) + "</ul>"
    # Geminiへの質問プロンプトを作成
    situation_prompt = f"""
